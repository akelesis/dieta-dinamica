...
export async function accessForUser(admin: SupabaseClient, user: User): Promise<AccessEntitlement> {
  const [billingResult, subscriptionResult, betaResult] = await Promise.all([
    ...
    admin.from('subscriptions').select('status, plan_mode, current_period_start, current_period_end').eq('user_id', user.id).maybeSingle(),
    user.email
  ...
  if (!billingResult.data?.enabled) return { billingEnabled: false, planMode: null, source: 'billing_disabled', periodStart: null, periodEnd: null }

  const subscription = subscriptionResult.data
  if (subscription && ['active', 'trialing'].includes(subscription.status)) {
    return { billingEnabled: true, planMode: subscription.plan_mode as AccessPlan, source: 'subscription', periodStart: subscription.current_period_start, periodEnd: subscription.current_period_end }
  }

  const beta = betaResult.data
  const betaCurrent = beta?.active && (!beta.expires_at || new Date(beta.expires_at).getTime() > Date.now())
  if (betaCurrent) return { billingEnabled: true, planMode: beta.plan_mode as AccessPlan, source: 'beta', periodStart: null, periodEnd: beta.expires_at }
  return { billingEnabled: true, planMode: null, source: null, periodStart: null, periodEnd: null }
}
