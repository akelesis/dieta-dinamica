...
export interface NutritionistReviewContext {
  profile: { age: number; sex: 'female' | 'male'; reproductiveStatus?: string; height: number; weight: number; goal: string; dailyActivity: string; workoutsPerWeek: number; workoutMinutes: number; intensity: string }
  nutrition: { dailyTarget?: number; baseTarget?: number; workoutCaloriesPerSession?: number; weeklyWorkoutCalories?: number; averageWorkoutCalories?: number; reproductiveCalories?: number; restTarget?: number; activeTarget?: number; protein: number; carbs: number; fat: number }
