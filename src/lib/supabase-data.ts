...
function profileFromRow(row: Record<string, unknown>): Profile {
  return {
    ...
    reproductiveStatus: (row.reproductive_status || 'none') as Profile['reproductiveStatus'],
...
function planFromRow(row: Record<string, unknown>): PlanPreferences {
  return {
    ...
    workoutTime: row.workout_time ? String(row.workout_time).slice(0, 5) : '',
    breakfastAvailability: (row.breakfast_availability || 'comfortable') as PlanPreferences['breakfastAvailability'],
    lunchAvailability: (row.lunch_availability || 'comfortable') as PlanPreferences['lunchAvailability'],
    dinnerAvailability: (row.dinner_availability || 'comfortable') as PlanPreferences['dinnerAvailability'],
...
export async function upsertProfile(userId: string, profile: Profile) {
  const { error } = await client().from('profiles').upsert({
    ...
    sex: profile.sex,
    reproductive_status: profile.sex === 'female' ? profile.reproductiveStatus : 'none',
    goal: profile.goal,
...
export async function upsertPlanPreferences(userId: string, plan: PlanPreferences) {
  const { error } = await client().from('plan_preferences').upsert({
    ...
    workout_time: plan.workoutTime || null,
    breakfast_availability: plan.breakfastAvailability,
    lunch_availability: plan.lunchAvailability,
    dinner_availability: plan.dinnerAvailability,
