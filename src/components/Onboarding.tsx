...
export function Onboarding({ onComplete, initialProfile, cloudStorage = false, onSignOut }: Props) {
  ...
  const reproductiveCategory = data.reproductiveStatus.startsWith('pregnant_') ? 'pregnant' : data.reproductiveStatus.startsWith('breastfeeding_') ? 'breastfeeding' : 'none'
  const setReproductiveCategory = (category: 'none' | 'pregnant' | 'breastfeeding') => set('reproductiveStatus', category === 'pregnant' ? 'pregnant_first_trimester' : category === 'breastfeeding' ? 'breastfeeding_0_6_months' : 'none')
        ...
        {step === 1 && (
          <section className="onboarding-card">
            ...
            <div className="form-grid">
              <label className="field body-sex-field"><span>Sexo biológico <small>(para o cálculo)</small></span><select value={data.sex} onChange={event => { const sex = event.target.value as BiologicalSex; setData(current => ({ ...current, sex, reproductiveStatus: sex === 'female' ? current.reproductiveStatus : 'none' })) }}><option value="female">Feminino</option><option value="male">Masculino</option></select></label>
              {data.sex === 'female' && <section className="reproductive-status-panel" aria-labelledby="reproductive-status-title"><div className="reproductive-status-heading"><span><Baby size={18} /></span><div><strong id="reproductive-status-title">Você está gestante ou amamentando?</strong><small>Essa informação ajusta sua estimativa calórica diária.</small></div></div><div className="reproductive-category-options">{([['none', 'Não'], ['pregnant', 'Gestante'], ['breastfeeding', 'Amamentando']] as const).map(([category, label]) => <button type="button" key={category} aria-pressed={reproductiveCategory === category} className={reproductiveCategory === category ? 'selected' : ''} onClick={() => setReproductiveCategory(category)}>{reproductiveCategory === category && <Check size={14} />}{label}</button>)}</div>{reproductiveCategory === 'pregnant' && <label className="field"><span>Trimestre da gestação</span><select value={data.reproductiveStatus} onChange={event => set('reproductiveStatus', event.target.value as ReproductiveStatus)}><option value="pregnant_first_trimester">1º trimestre</option><option value="pregnant_second_trimester">2º trimestre</option><option value="pregnant_third_trimester">3º trimestre</option></select></label>}{reproductiveCategory === 'breastfeeding' && <label className="field"><span>Idade do bebê</span><select value={data.reproductiveStatus} onChange={event => set('reproductiveStatus', event.target.value as ReproductiveStatus)}><option value="breastfeeding_0_6_months">Até 6 meses</option><option value="breastfeeding_7_12_months">De 7 a 12 meses</option></select></label>}<small className="reproductive-status-note">Gestação e amamentação exigem acompanhamento individualizado com obstetra e nutricionista.</small></section>}
