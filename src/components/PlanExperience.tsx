...
import type { DietaryStyle, FoodBudget, GeneratedMeal, GeneratedPlanResponse, HealthCondition, MealAvailability, NutritionPlan, PlanMode, PlanPreferences, Profile, Subscription } from '../types'
...
const availabilityOptions: { id: MealAvailability; title: string; text: string }[] = [
  { id: 'comfortable', title: 'Com tranquilidade', text: '30 minutos ou mais' },
  { id: 'limited', title: 'Tempo contado', text: 'Entre 15 e 30 minutos' },
  { id: 'very_limited', title: 'Muito corrido', text: 'Até 15 minutos ou no trabalho' },
]
...
function PlanOnboarding({ profile, onComplete, forcedMode }: Pick<Props, 'profile' | 'onComplete'> & { forcedMode?: PlanMode }) {
  ...
  const [data, setData] = useState<Omit<PlanPreferences, 'completedAt'>>({
    planMode: forcedMode || 'self', dietaryStyle: 'omnivore', mealsPerDay: 4, restrictions: [], favoriteFoods: '', dislikedFoods: '', cookingTime: 'moderate', budget: 'balanced', breakfastTime: '07:30', lunchTime: '12:30', dinnerTime: '19:30', workoutTime: profile.workoutsPerWeek > 0 ? '18:00' : '', breakfastAvailability: 'comfortable', lunchAvailability: 'comfortable', dinnerAvailability: 'comfortable', hasHealthCondition: false, healthConditions: [], healthNotes: '',
  })
  ...
  const canContinue = (step !== 1 || !data.hasHealthCondition || data.healthConditions.length > 0) && (step !== 3 || profile.workoutsPerWeek === 0 || Boolean(data.workoutTime))
  ...
  return (
    <section className="plan-onboarding">
      ...
      <div className="plan-progress"><div><span>Etapa {step + 1} de 5</span><b>{['Alimentação', 'Cuidados', 'Rotina', 'Disponibilidade', 'Horários'][step]}</b></div><div className="plan-progress-track"><i style={{ width: `${(step + 1) * 20}%` }} /></div></div>
      ...
      <div className="plan-question-card">
        ...
        {step === 3 && <div className="plan-step"><span className="plan-step-icon"><Activity size={23} /></span><h2>Como treino e refeições cabem no seu dia?</h2><p>Isso ajuda a posicionar refeições práticas e adequadas ao momento do treino.</p>{profile.workoutsPerWeek > 0 ? <label className="field workout-time-field"><span><Activity size={15} /> Horário em que costuma treinar</span><input type="time" value={data.workoutTime} onChange={event => set('workoutTime', event.target.value)} /><small>Informe o horário mais comum. Ele será usado como referência, sem mudar sua meta diária.</small></label> : <div className="no-workout-note"><Activity size={17} /><span>Seu perfil está sem treinos semanais. Você pode atualizar essa informação em Meu perfil quando sua rotina mudar.</span></div>}<div className="meal-availability-grid">{([['Café da manhã', 'breakfastAvailability'], ['Almoço', 'lunchAvailability'], ['Jantar', 'dinnerAvailability']] as const).map(([label, key]) => <section className="meal-availability-card" key={key}><div><strong>{label}</strong><small>Quanto tempo você normalmente tem para comer?</small></div><div className="availability-choice">{availabilityOptions.map(option => <button type="button" key={option.id} className={data[key] === option.id ? 'selected' : ''} onClick={() => set(key, option.id)}><b>{option.title}</b><span>{option.text}</span></button>)}</div></section>)}</div><div className="routine-context-note"><Info size={17} /><span>Refeições com pouco tempo serão mais simples, portáteis ou preparadas com antecedência. Quando necessário, parte das calorias e macros será distribuída para uma refeição posterior.</span></div></div>}

        {step === 4 && <div className="plan-step"><span className="plan-step-icon"><Clock3 size={23} /></span><h2>Últimos detalhes da sua rotina</h2><p>Use horários aproximados. O plano continua flexível.</p><div className="time-grid"><label className="field"><span>Café da manhã</span><input type="time" value={data.breakfastTime} onChange={event => set('breakfastTime', event.target.value)} /></label><label className="field"><span>Almoço</span><input type="time" value={data.lunchTime} onChange={event => set('lunchTime', event.target.value)} /></label><label className="field"><span>Jantar</span><input type="time" value={data.dinnerTime} onChange={event => set('dinnerTime', event.target.value)} /></label></div><div className="preference-grid"><label className="field"><span>Alimentos que você gosta</span><textarea rows={3} placeholder="Ex.: arroz, feijão, banana, frango..." value={data.favoriteFoods} onChange={event => set('favoriteFoods', event.target.value)} /></label><label className="field"><span>Alimentos que prefere evitar</span><textarea rows={3} placeholder="Ex.: brócolis, peixe, abacate..." value={data.dislikedFoods} onChange={event => set('dislikedFoods', event.target.value)} /></label></div><div className="plan-ready-note">{data.planMode === 'self' ? <Calculator size={18} /> : <Sparkles size={18} />}<div><strong>{data.planMode === 'self' ? 'Sua estrutura está pronta' : 'Tudo pronto para personalizar'}</strong><span>{data.planMode === 'self' ? 'Agora você poderá preencher e editar cada refeição por conta própria.' : 'Vamos combinar seu objetivo, gasto energético, treino, disponibilidade e preferências.'}</span></div></div></div>}
      </div>

      <div className="plan-onboarding-actions"><button className="button ghost" disabled={step === 0 && Boolean(forcedMode)} onClick={() => step === 0 ? setModeChosen(false) : setStep(value => value - 1)}><ArrowLeft size={17} /> Voltar</button>{step < 4 ? <button className="button primary" disabled={!canContinue} onClick={() => setStep(value => value + 1)}>Continuar <ArrowRight size={17} /></button> : <button className="button primary" onClick={finish}>{data.planMode === 'self' ? <>Abrir meu planejador <Calculator size={17} /></> : <>Construir meu plano <Sparkles size={17} /></>}</button>}</div>
    </section>
  )
}
...
function PersonalizedPlan({ profile, nutrition, preferences }: Omit<Props, 'preferences' | 'onComplete'> & { preferences: PlanPreferences }) {
  ...
  const [selectedMenuId, setSelectedMenuId] = useState<'A' | 'B' | 'C'>('A')
  ...
  const selectedMenu = plan?.menus.find(menu => menu.id === selectedMenuId) || plan?.menus[0]
  const approved = response?.reviewStatus === 'approved'
  const calorieTarget = nutrition.dailyTarget
  const currentCalories = selectedMenu?.dailyCalories ?? calorieTarget
  ...
  return <>
    ...
    <section className="plan-hero card"><div><span className="card-kicker"><ShieldCheck size={16} /> {approved ? 'Plano profissionalmente aprovado' : 'Rascunho aguardando aprovação'}</span><h2>{currentCalories.toLocaleString('pt-BR')} <small>kcal / dia</small></h2><p>{plan?.summary || <>Meta estável nos sete dias, calculada com a média semanal dos treinos: <strong>{nutrition.dailyTarget.toLocaleString('pt-BR')} kcal por dia</strong>.</>}</p><span className="calorie-band-note">Você recebeu três cardápios completos para alternar durante a rotina.</span></div><div className="macro-summary"><div><strong>{selectedMenu?.protein ?? nutrition.protein}g</strong><span>proteínas</span></div><div><strong>{selectedMenu?.carbs ?? nutrition.carbs}g</strong><span>carboidratos</span></div><div><strong>{selectedMenu?.fat ?? nutrition.fat}g</strong><span>gorduras</span></div></div></section>
    ...
    <section className="meal-plan"><div className="section-heading"><div><span className="section-icon"><Salad size={19} /></span><div><h2>Seus cardápios</h2><p>Alterne entre A, B e C. Quantidades referem-se aos alimentos prontos ou cozidos, salvo quando indicado.</p></div></div>{response && <span className={`review-status ${response.reviewStatus || 'pending'}`}>{response.reviewStatus === 'approved' ? 'Aprovada' : response.reviewStatus === 'in_review' ? 'Em revisão' : 'Na fila'}</span>}</div>{plan && <div className="menu-option-tabs" role="tablist" aria-label="Opções de cardápio">{plan.menus.map(menu => <button key={menu.id} type="button" role="tab" aria-selected={menu.id === selectedMenu?.id} className={menu.id === selectedMenu?.id ? 'active' : ''} onClick={() => setSelectedMenuId(menu.id)}><span>Cardápio {menu.id}</span><small>{menu.dailyCalories.toLocaleString('pt-BR')} kcal</small></button>)}</div>}{loading && !plan ? <div className="plan-generation-state"><LoaderCircle className="spin" size={25} /><strong>Montando três cardápios para revisão…</strong><span>Calculando porções, calorias e macronutrientes antes do envio ao nutricionista.</span></div> : error && !plan ? <div className="plan-generation-state error"><AlertCircle size={25} /><strong>Não conseguimos solicitar o plano</strong><span>{error}</span><button className="button secondary" onClick={retry}>Tentar novamente</button></div> : selectedMenu && <div className={`generated-plan-list ${loading ? 'updating' : ''}`}>{selectedMenu.meals.map(meal => <DetailedMeal key={`${selectedMenu.id}-${meal.label}-${meal.time}`} meal={meal} approved={approved} />)}</div>}{error && plan && <div className="inline-plan-error"><AlertCircle size={15} /> {error}</div>}</section>
